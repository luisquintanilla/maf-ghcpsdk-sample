...
internal static class TaxTools
{
    ...
    public static string OptimizeAssetLocation()
    {
        ...
        // High dividends in Traditional (acct=2) → low score; in Taxable (acct=0) → high score
        // High growth in Roth (acct=1) → low score; elsewhere → higher score
        // Bonds in Traditional → low score
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            // Taxable: pay tax on dividends → cost = divScore + bondBonus
            var taxableCost = ctx.MkInt(divScore + bondBonus);
            // Roth: growth is tax-free → cost = divScore - growthScore (lower for high growth)
            var rothCost = ctx.MkInt(Math.Max(0, divScore - growthScore));
            // Traditional: dividends sheltered → cost = -divScore - bondBonus (negative = good)
        ...
        if (opt.Check() == Status.SATISFIABLE)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                // Estimate annual tax drag (simplified: dividends taxed at 22% in taxable)
        ...
        [Description("Target harvest amount in dollars, or 0 to maximize")] double targetAmount = 0)
    {
        ...
        // Wash sale constraints: if selling a lot, no lot of the same symbol
        // should have been purchased within 30 days before or after today
        var washSaleWarnings = new List<object>();
        for (int i = 0; i < m; i++)
        {
            var lot = lossingLots[i];
            // Check if any lot of the same symbol was purchased within 30 days of today
            ...
            {
                // Block this lot from being sold (wash sale risk)
    ...
    public static string ComputeTaxSavings()
    {
        ...
        float dividendSavings = dividendTaxDrag * 0.7f;   // ~70% of dividends could be sheltered
        float growthSavings = growthTaxDrag * 0.5f;        // ~50% of growth could go to Roth
        float bondSavings = bondTaxDrag;                   // bonds should be fully in Traditional
    ...
    {
        // Recompute savings for the chart
